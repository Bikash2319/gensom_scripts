...
for item in data_list:
    ...
    random_identity = ''.join(np.random.choice(characters, 10))
    ...
    project_option = wait.until(EC.element_to_be_clickable(driver.find_element(By.XPATH, "//ng-dropdown-panel//div[contains(@class, 'ng-option') and span[text()='AIR DELHI']]")))
    project_option.click()
    time.sleep(0.5)

    enrollment_dd = Select(driver.find_element(By.XPATH, "//select[@formcontrolname='enrollment']"))
    enrollment_dd.select_by_visible_text("EXTERNAL")
    time.sleep(0.5)

    role_dd = Select(driver.find_element(By.XPATH, "//select[@formcontrolname='role_type']"))
    role_dd.select_by_visible_text("SERVICE PROVIDER")
    time.sleep(0.5)

    skills = 'Technician'
    driver.find_element(By.XPATH, "//input[@formcontrolname='skill_id']").send_keys(skills)
